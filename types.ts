...
// Basic Types based on eSocial Manual
export enum Environment {
  Production = 1,
  ProductionRestricted = 2, // Test environment
}
...
export type FieldType = 'text' | 'number' | 'date' | 'select' | 'email' | 'tel' | 'month' | 'textarea' | 'time';
...
export interface FormField {
  ...
  label: string;
  type: FieldType;
  path: string; // JSON path for state mapping e.g. 'infoEmpregador.inclusao.infoCadastro.classTrib'
  required?: boolean;
  options?: FieldOption[];
  placeholder?: string;
  description?: string;
  group?: string; // For visual grouping in the form
  maxLength?: number;
}

export interface EventSchema {
  id: string; // e.g., 'S-1000'
  title: string;
  description: string;
  fields: FormField[];
  defaultState: any;
...
// Complete list of eSocial Events based on Manual v1.3
...
export interface S1000Data {
  tpAmb: Environment | number | string;
  procEmi: number | string;
  verProc: string;
  tpInsc: InscriptionType | number | string;
  nrInsc: string;
  dtIni: string;
  iniValid?: string;
  acao: string;
  classTrib: string;
  indCoop: number | string;
  indConstr: number | string;
  indDesFolha: number | string;
  nmCtt: string;
  cpfCtt: string;
  email: string;
  foneFixo?: string;
